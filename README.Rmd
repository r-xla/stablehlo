...
It also supports selected ops from the [CHLO](https://openxla.org/stablehlo/generated/chlo) dialect, a higher-level companion to stableHLO that is lowered to stableHLO during compilation.
These programs can be executed using the R package [pjrt](https://github.com/r-xla/pjrt).

{stablehlo} is the IR layer of the [r-xla](https://github.com/r-xla) ecosystem. The main user-facing package is [{anvl}](https://github.com/r-xla/anvl), a code transformation framework for R that builds on {stablehlo} and {pjrt} to provide JIT compilation and automatic differentiation. Most users should reach for {anvl} directly; {stablehlo} is intended for those who want to construct or manipulate stableHLO programs at the IR level.
...
Below, we create a function that takes two input arguments `x` and `y` of type `f32` and shape `2x2` and adds them.
...
* Supports a subset of [CHLO](https://openxla.org/stablehlo/generated/chlo) ops (e.g. inverse trig, hyperbolic, gamma family) that are not StableHLO primitives and would otherwise have to be hand-composed.
...
* We try to use `"bool"` for the boolean dtype. However, in the generated stableHLO
  representation this will show up as `"i1"`. Think of these as interchangeable.
