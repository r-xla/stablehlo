...
test_that("unify_shapes refines toward the known side", {
  expect_equal(unify_shapes(c(N, 3L), c(2L, N)), c(2L, 3L))
  expect_equal(unify_shapes(c(N, N), c(N, N)), c(N, N))
  expect_equal(unify_shapes(c(2L, 3L), c(2L, 3L)), c(2L, 3L))
  expect_equal(unify_shapes(integer(), integer()), integer())
})

test_that("unify_shapes errors only on a definite clash", {
  expect_error(unify_shapes(c(2L, 3L), c(2L, 4L)), "dimension")
  expect_no_error(unify_shapes(c(2L, N), c(2L, 4L)))
  expect_error(unify_shapes(c(2L, 3L), 2L), "same rank")
})

test_that("unify_all_shapes folds, so a set that cannot agree is caught", {
  ...
  expect_error(unify_all_shapes(list(N, 3L, 4L)), "dimension")
  expect_equal(unify_all_shapes(list(N, 3L, N)), 3L)
  expect_equal(unify_all_shapes(list(c(N, 2L), c(5L, N))), c(5L, 2L))
})
...
test_that("unify_vt refines a tensor type", {
  dyn <- ValueType(TensorType(as_dtype("f32"), Shape(c(N, 3L))))
  sta <- ValueType(TensorType(as_dtype("f32"), Shape(c(2L, N))))
  expect_equal(repr(unify_vt(dyn, sta)), "tensor<2x3xf32>")
  expect_equal(repr(unify_vt(dyn, dyn)), "tensor<?x3xf32>")
})

test_that("unify_vt still rejects a dtype, rank or definite size mismatch", {
  ...
  # index; `unify_shapes()`'s per-axis message is for the fold sites.
  expect_error(unify_vt(a, b), "same tensor type")
  expect_error(unify_vt(a, c2), "same tensor type")
  expect_error(unify_vt(d, e), "same tensor type")
})
