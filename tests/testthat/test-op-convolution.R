...
# Asymmetric (causal) padding: pads only the low edge, and by a different
# amount per spatial dimension. The padding matrix is [n_spatial, 2] with
# (low, high) per row, so this pins down the row/column orientation of the
# `dense<[[low, high], ...]>` rendering -- a transposition would be invisible
# for the symmetric padding the other tests use.
test_that("2D convolution with asymmetric (causal) padding", {
  func <- local_func()

  lhs <- hlo_input("lhs", "f32", shape = c(1L, 3L, 3L, 1L))
  rhs <- hlo_input("rhs", "f32", shape = c(2L, 2L, 1L, 1L))

  # height: (low = 1, high = 0); width: (low = 2, high = 0)
  padding <- matrix(c(1L, 0L, 2L, 0L), nrow = 2L, byrow = TRUE)
  ...
  # padded height 3 + 1 = 4, padded width 3 + 2 = 5, 2x2 window, stride 1
  # -> output spatial dims 3 x 4
  expect_equal(shape(z), c(1L, 3L, 4L, 1L))
  expect_snapshot(repr(f))
  ...
  x <- array(0, dim = c(1L, 3L, 3L, 1L))
  x[1L, , , 1L] <- matrix(1:9, nrow = 3L, byrow = TRUE)
  k <- array(1, dim = c(2L, 2L, 1L, 1L))
  out <- pjrt_execute(exec, pjrt_buffer(x, "f32"), pjrt_buffer(k, "f32"))

  # sum over each 2x2 window of the zero-padded input:
  #   0 0 0 0 0
  #   0 0 1 2 3
  #   0 0 4 5 6
  #   0 0 7 8 9
  expected <- array(0, dim = c(1L, 3L, 4L, 1L))
  expected[1L, , , 1L] <- rbind(
    c(0, 1, 3, 5),
    c(0, 5, 12, 16),
    c(0, 11, 24, 28)
  )
  expect_equal(out, pjrt_buffer(expected, "f32"))
})
...
test_that("error: a zero-sized kernel spatial dimension gives no window", {
  local_func()
  lhs <- hlo_input("lhs", "f32", shape = c(1L, 5L, 5L, 1L))
  rhs <- hlo_input("rhs", "f32", shape = c(0L, 3L, 1L, 1L))
  expect_snapshot_error({
    hlo_convolution(
      lhs,
      rhs,
...
test_that("a zero-sized input spatial dimension is still allowed", {
  local_func()
  lhs <- hlo_input("lhs", "f32", shape = c(1L, 0L, 5L, 1L))
  rhs <- hlo_input("rhs", "f32", shape = c(3L, 3L, 1L, 1L))
  z <- hlo_convolution(
    lhs,
    rhs,
