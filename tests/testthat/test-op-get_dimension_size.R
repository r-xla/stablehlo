...
test_that("dynamic axis", {
  ...
  # The result is a scalar i32 whatever the queried axis is -- that is the
  # point of the op, and it is what needs asserting. Matching `tensor<?xf32>`
  # only finds `%x` in the signature, so it would hold even if the operand's
  # `?` leaked into the result.
  expect_equal(repr(y$value_type$type), "tensor<i32>")
  f <- hlo_return(y)
  expect_match(repr(f), "tensor<?xf32>) -> tensor<i32>", fixed = TRUE)
})
...
test_that("the dimension must be in range", {
  ...
  expect_error(
    hlo_get_dimension_size(x, dimension = 1L),
