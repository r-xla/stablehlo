...
infer_types_if <- function(pred, true_branch, false_branch) {
  ...
  # passes its branches nothing, so a branch declaring inputs renders
  # `^bb0(%x: ...)` inside `stablehlo.if` and MLIR refuses the region with
  # "expected 0 arguments". Nothing downstream catches it -- `pjrt_program()`
  # does not validate the text -- so it is checked here, as `case` (C2)
  # already checks for its own branches.
