...
# Generic-format render with the regular attrs plus the two custom
# attrs (dimension_numbers, precision_config) appended to the block.
render_convolution <- function(ctx) {
  attrs_parts <- vapply(
    ctx$attrs,
    function(a) repr(a, simplify_dense = TRUE),
    character(1)
  )
  dim_num <- ctx$custom_attrs$dimension_numbers
  precision <- ctx$custom_attrs$precision_config
  all_parts <- c(
    attrs_parts,
...
infer_types_convolution <- function(
  ...
  # Accept a plain character vector (e.g. c("DEFAULT", "DEFAULT")) as a
  # convenience, matching hlo_convolution()'s precision_config argument.
  if (is.character(precision_config)) {
...
#'   Output types known ahead of time (e.g. from type inference at trace time).
#'   When provided, type inference and its input validation are skipped.
#' @return [`FuncValue`]
#' @export
